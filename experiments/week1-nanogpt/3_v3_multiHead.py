...
here we are going a step further from v2 file and implementing multi-head attention mechanism.
═══════════════════════════════════════════════════════════════════════════════
"""
...
# ════════════════════════════════════════════════════════════════════════════════════════════════
# MULTI-HEAD ATTENTION - Let the model look at multiple aspects of the context simultaneously
# ════════════════════════════════════════════════════════════════════════════════════════════════

# What is each line of code doing below?
# Does all the computation happen in paralle here?
class MultiHeadAttention(nn.Module):
    """ Multiple heads of self-attention in parallel """

    def __init__(self, num_heads, head_size):
        super().__init__() # ?
        self.heads = nn.ModuleList([Head(head_size) for _ in range(num_heads)]) # What is line doing? What is ModuleList and Head() doing in this ? I am guessing this is making multiple heads

    
    def forward(self, x):
        return torch.cat([h(x) for h in self.heads], dim = -1) # What is dimension doing here? What is this line doing? # Are we concatinating all the Channel dimension if so why?
...
class BigramLanguageModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.sa_head = MultiHeadAttention(4, n_embd//4) # i.e r heads of 8 dimensional self Attention # How is this 8 dimension?  # This is kind of like group convolution
        # We have 4 communicate channels with smaller head size which is 8
        # Why can't we have 32 channels here? What's the logic here?
